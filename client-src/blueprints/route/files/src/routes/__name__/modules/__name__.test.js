...
import {
  ...
  getQuote,
  receiveQuote,
  <%= camelEntityName %>Reducer,
  <%= camelEntityName %>Selector,
...
test('(Constant) UPDATE_TITLE === "<%= pascalEntityName %>/UPDATE_TITLE"', t => {
  t.is(UPDATE_TITLE, '<%= pascalEntityName %>/UPDATE_TITLE');
});

test('(Constant) GET_QUOTE === "<%= pascalEntityName %>/GET_QUOTE"', t => {
  t.is(GET_QUOTE, '<%= pascalEntityName %>/GET_QUOTE');
});

test('(Constant) RECEIVE_QUOTE === "<%= pascalEntityName %>/RECEIVE_QUOTE"', t => {
  t.is(RECEIVE_QUOTE, '<%= pascalEntityName %>/RECEIVE_QUOTE');
});
...
test('(Reducer) initializes with title state', t => {
  t.deepEqual(<%= camelEntityName %>Reducer(undefined, {}), { title: 'A Title Stored in global state.' });
});
...
test('(Saga) watchGetQuote: no quote received.', t => {
  ...
  t.deepEqual(saga.next().value, select(<%= camelEntityName %>Selector),
