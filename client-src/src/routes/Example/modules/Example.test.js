...
test('(Constant) UPDATE_TITLE === "Example/UPDATE_TITLE"', t => {
  t.is(UPDATE_TITLE, 'Example/UPDATE_TITLE');
});

test('(Constant) GET_QUOTE === "Example/GET_QUOTE"', t => {
  t.is(GET_QUOTE, 'Example/GET_QUOTE');
});

test('(Constant) RECEIVE_QUOTE === "Example/RECEIVE_QUOTE"', t => {
  t.is(RECEIVE_QUOTE, 'Example/RECEIVE_QUOTE');
});
...
test('(Reducer) initializes with title state', t => {
  t.deepEqual(exampleReducer(undefined, {}), { title: 'A Title Stored in global state.' });
});
...
test('(Saga) watchGetQuote: no quote received.', t => {
  ...
  t.deepEqual(saga.next().value, select(exampleSelector),
