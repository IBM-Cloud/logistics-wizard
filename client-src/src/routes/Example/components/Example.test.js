...
const setup = (quote) => {
  ...
  const component = shallow(<Example {...props} />);
...
test('(Component) Renders with expected elements', t => {
  ...
  t.true(component.is('div'),
    'is wrapped by a div');
  t.true(component.hasClass('example'),
    'uses class "example" in wrapper div');
  t.is(component.find('h4').length, 2,
    'has two h4 elements');
  t.is(component.find('h4').first().text(), `Example - ${props.title}`,
